...
FIXTURE="../Tests/PowerpointViewerTests/Fixtures/fixture.pptx"

echo "▶ [1/5] Smoke-testing the app (window, open, present)…"
# Catches startup crashes — the app is a GUI binary, so a failure here would
# otherwise ship silently and simply never open a window on the user's machine.
if [ -n "${SKIP_SMOKE:-}" ]; then
  echo "    skipped (SKIP_SMOKE set)"
elif dotnet run --project SlideViewer -v q -- --selftest "$FIXTURE" 2>&1 | grep -q "SELFTEST OK"; then
  echo "    OK"
else
  echo "✗ App failed to start — refusing to package a broken build."
  echo "  Re-run for details: dotnet run --project SlideViewer -- --selftest $FIXTURE"
  exit 1
fi

echo "▶ [2/5] Publishing self-contained win-x64 application…"
rm -rf "$PUBLISH_DIR"
...
echo "▶ [3/5] Packing application payload…"
